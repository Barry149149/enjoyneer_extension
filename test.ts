...
namespace dht11_dht22 {
    ...
    function queryData(dataPin: DigitalPin): number[] {
        ...
        if (pins.digitalReadPin(dataPin) != 1) {
            while (pins.digitalReadPin(dataPin) == 0 || pins.digitalReadPin(dataPin) == 1); //sensor response
            ...
            if ((resultArray[0] + resultArray[1] + resultArray[2] + resultArray[3]) % 256 == resultArray[4]) {
                //DHT11
                return [resultArray[2] + resultArray[3] / 10, resultArray[0] + resultArray[1] / 100]
            }
        }
        return [-1, -1]
    }
    ...
    //% block="DHT11 Humidity |pin $dataPin| unit $tempType"
    export function Humidity(dataPin: DigitalPin) {
        return queryData(dataPin)[1]
    }
}
